...
tasks 数组中每一条已确认的任务对应一条 items 记录（条数须与 tasks 一致）；timeStart、timeEnd 为 24 小时制，且 timeEnd 晚于 timeStart。`;
  ...
  function getSortedListItems() {
    ...
    if (getSheetKey() === "daily") {
      return sheet.items.slice().sort((a, b) => {
        ...
        const ea = timeToMinutes(a.timeEnd) ?? Infinity;
        const eb = timeToMinutes(b.timeEnd) ?? Infinity;
  ...
  function findConflictsInItems(items) {
    ...
    const list = items
      .map((it) => {
        const iv = itemTimeInterval(it);
        return iv ? { ...iv, item: it } : null;
      })
      .filter(Boolean);
  ...
  function findConflictWithId(items, id) {
    ...
    if (!self) return null;
    const ivSelf = itemTimeInterval(self);
    if (!ivSelf) return null;
    for (const other of items) {
      if (other.id === id) continue;
      const ivO = itemTimeInterval(other);
      if (!ivO) continue;
      if (Math.max(ivSelf.start, ivO.start) < Math.min(ivSelf.end, ivO.end)) {
        return { other, self };
      }
    }
    return null;
  ...
  function resolveOverlapsByClearingLater(items) {
    ...
    const withIntervals = result
      .map((it) => {
        const iv = itemTimeInterval(it);
        return iv ? { ...iv, id: it.id } : null;
      })
      .filter(Boolean)
      .sort((a, b) => a.start - b.start || a.end - b.end);
    const kept = [];
    for (const seg of withIntervals) {
      let bad = false;
      for (const k of kept) {
        if (Math.max(seg.start, k.start) < Math.min(seg.end, k.end)) {
          bad = true;
          break;
        }
      }
  ...
  function draftTimeModeLabel(mode) {
    return mode === "fragment" ? "可碎片化时间完成" : "必须整块时间完成";
  }
  ...
  function renderDailySmartWizard() {
    ...
    if (sp.step === 1) {
      ...
      const hint = document.createElement("p");
      hint.className = "smart-ai-hint";
      hint.textContent =
        "点击下方按钮后，将向你在服务器上配置的接口请求生成安排，不会在页面中填写或保存 API 密钥。";
      ...
      aiBlock.append(hint, errBox, aiActions);
  ...
  function buildItemsFromAiParsed(sp, parsed) {
    ...
    for (let i = 0; i < confirmedDrafts.length; i++) {
      ...
      if (looksLikeModelScheduleRefusal(String(row.timeStart)) || looksLikeModelScheduleRefusal(String(row.timeEnd))) {
        throw new Error("模型返回的时间字段无效（疑似说明文字而非时刻）。请重试。");
      }
      const timeStart = parseTimeHHMM(String(row.timeStart).trim());
      const timeEnd = parseTimeHHMM(String(row.timeEnd).trim());
      if (!timeStart || !timeEnd) {
        throw new Error("返回的时间格式无效，应为 HH:MM。");
      }
      const ts = timeToMinutes(timeStart);
      const te = timeToMinutes(timeEnd);
  ...
  function patchCurrentSheetItem(id, patch) {
    const sheet = getSheet();
    sheet.items = sheet.items.map((t) => (t.id === id ? { ...t, ...patch } : t));
    save();
  }
  ...
  tagForm.addEventListener("submit", (e) => {
    ...
    sheet.categories.push({ id: uid(), name });
    tagInput.value = "";
    save();
    render();
  });
